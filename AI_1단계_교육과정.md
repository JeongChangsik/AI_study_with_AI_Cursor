...
- 날짜별 자료에는 링크 목록만 두지 않고, 반드시 **"오늘 꼭 알아야 할 핵심 설명(서술형)"** 섹션을 포함합니다.
...
| 6단계. 책임있는 AI/데이터·모델 보안 | 안전하고 신뢰할 수 있는 AI를 만들려면? | 1주 | 편향/프라이버시/보안 리스크 설명 가능 |
| 7단계. 내 AI 개발 시작 | 이론을 내 프로젝트로 연결하려면? | 2주 | 베이스라인 + 개선 계획 작성 |

권장 총 학습 기간: **17주 (주당 8~10시간)**  
...
### 2-0) 단계별 꼭 알아야 할 핵심 설명 (서술형 요약)

AI 학습의 출발점은 알고리즘 이름을 많이 외우는 것이 아니라, "내가 어떤 문제를 풀고 싶은지"를 명확히 말하는 능력입니다.  
1단계에서는 AI, ML, DL의 관계를 정확히 구분하고, 데이터-모델-평가 지표가 어떻게 연결되는지 이해해야 이후 학습이 흔들리지 않습니다.

2단계 수학/통계는 단순한 배경지식이 아니라 모델이 왜 그렇게 동작하는지 설명해주는 언어입니다.  
벡터와 행렬은 데이터를 표현하는 도구이고, 미분과 경사하강법은 모델이 오차를 줄이며 학습되는 원리를 설명합니다.

3단계 머신러닝에서는 "정확도가 높다"는 말만으로는 충분하지 않다는 사실을 배우는 것이 핵심입니다.  
문제 특성에 맞는 평가 지표를 고르고, 데이터 누수와 과적합을 방지해야 실제 환경에서도 성능이 유지됩니다.

4단계 딥러닝은 표현 학습의 힘을 이해하는 구간입니다.  
신경망이 층을 거치며 특징을 자동으로 추출하는 구조, 역전파로 가중치를 갱신하는 원리, 아키텍처별 강점(CNN/Transformer)을 흐름으로 이해해야 합니다.

5단계 생성형 AI/LLM에서는 "다음 토큰 예측"이라는 단순한 목표가 왜 강력한 능력으로 이어지는지 이해하는 것이 중요합니다.  
동시에 환각, 근거 부족, 평가 난이도 같은 한계를 같이 학습해야 실무에서 무리한 기대를 줄일 수 있습니다.

6단계 책임있는 AI는 선택이 아닌 필수입니다.  
편향, 개인정보, 저작권, 보안 리스크를 초기에 점검하지 않으면 모델 품질이 좋아도 서비스 품질은 실패할 수 있습니다.

7단계는 이론을 프로젝트로 연결하는 단계입니다.  
문제정의 -> 베이스라인 -> 오류 분석 -> 개선 실험의 반복 루프를 문서화하며 진행해야, 학습과 개발이 모두 남는 자산이 됩니다.
...
### 6단계. 책임있는 AI/데이터·모델 보안 (중요 보강)

**반드시 이해할 개념**
- 데이터 프라이버시: 개인정보, 비식별화, 최소수집 원칙
- 공정성/편향: 데이터 편향, 샘플링 편향, 평가 편향
- 저작권/라이선스: 학습 데이터 출처 관리, 생성물 사용 정책
- 보안 기초: Prompt Injection, Data Poisoning, 모델 출력 유출 위험

**추천 자료 (무료)**
- [NIST AI Risk Management Framework](https://www.nist.gov/itl/ai-risk-management-framework)
- [Google Responsible AI](https://ai.google/responsibility/)
- [Fairness and Machine Learning (무료 공개서)](https://fairmlbook.org/)

**추천 도서**
- `Fairness and Machine Learning` (Barocas, Hardt, Narayanan)
- `Building Secure and Reliable Systems` (Google SRE Book Team)

**자기 점검 질문**
1. "정확도 높은 모델"이 실제 서비스에서 위험할 수 있는 이유는 무엇인가?
2. RAG 또는 파인튜닝 데이터에 라이선스 검토가 필요한 이유는?

---

### 7단계. 내 AI 개발 시작 (이론 -> 프로젝트 연결)
...
| 15주 | 책임있는 AI | 편향/프라이버시/라이선스/보안 체크리스트 학습 |
| 16~17주 | 내 프로젝트 | 문제 정의 -> 베이스라인 -> 개선 계획 수립 |
...
- [ ] 편향/프라이버시/라이선스/보안 리스크를 점검할 수 있다.
...
- [x] 2026-02-20: [AI_학습자료_2026-02-20.md](AI_학습자료_2026-02-20.md)
- [ ] 2026-02-21: 2일차(선형대수 기초) 예정
